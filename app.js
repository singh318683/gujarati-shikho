/* Gujarati Shikho v2: speaking-first prototype. No build step, no dependencies. */
(function () {
  'use strict';

  const C = window.COURSE;
  const CFG = window.APP_CONFIG || {};
  const KEY = CFG.storageKey || 'gujarati-shikho-v2';
  ...
  const scriptLine = (it) => (P.showScript ? '<p class="gu-line gu" lang="gu">' + esc(it.gu) + '</p>' : '');
  ...
  function feedback(ex, ok) {
    ...
    if (P.showScript) body += ' <span class="gu" lang="gu">' + esc(it.gu) + '</span>';
  ...
  function reviewView() {
    ...
    lessons.forEach((l) => {
      ...
      l.items.forEach((it) => {
        h += '<tr' + (it.check ? ' class="flag"' : '') + '><td>' + esc(it.rom) + '</td><td class="gu" lang="gu">' + esc(it.gu) + '</td><td>' + esc(it.en) + (it.check ? ' <strong>(please check)</strong>' : '') + '</td><td><code>' + it.id + '.mp3</code></td></tr>';
      });
      h += '</tbody></table></div>';
